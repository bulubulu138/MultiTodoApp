    ...
    try {
      ...
      if (diagnosticResult.success) {
        ...
        if (!diagnosticResult.healthy) {
          message.warning(`发现 ${diagnosticResult.recommendations.length} 个问题，建议查看详情`);
        } else {
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="large">
            ...
            <Row gutter={16} align="middle">
              ...
              <Col span={12}>
                <Statistic
                  ...
                  valueStyle={{ color: result.recommendations.length > 0 ? '#ff4d4f' : '#52c41a', fontSize: 24 }}
          ...
          <Collapse defaultActiveKey={['1', '2', '3', '4']} ghost>
            ...
            <Panel
              header={
                <Space>
                  {renderHealthIcon(result.checks.database.healthy)}
                  <Text strong>数据库状态</Text>
                  {renderHealthTag(result.checks.database.healthy)}
                </Space>
              }
              key="1"
            >
              <Descriptions bordered column={1}>
                ...
                <Descriptions.Item label="待办数量">
                  {result.checks.database.todoCount}
                </Descriptions.Item>
                <Descriptions.Item label="示例标题">
                  {result.checks.database.sampleTitles.length > 0
            ...
            <Panel
              header={
                <Space>
                  {renderHealthIcon(result.checks.fileSystem.healthy)}
                  <Text strong>文件系统状态</Text>
                  {renderHealthTag(result.checks.fileSystem.healthy)}
                </Space>
              }
              key="2"
            >
              <Descriptions bordered column={1}>
                ...
                <Descriptions.Item label="Markdown文件数量">
                  <Text
                    strong
                    style={{
                      color: result.checks.fileSystem.mdFileCount > 0 ? '#52c41a' : '#ff4d4f',
                      fontSize: 18
                    }}
                  >
                    {result.checks.fileSystem.mdFileCount}
                  </Text>
                </Descriptions.Item>
                <Descriptions.Item label="示例文件">
                  {result.checks.fileSystem.sampleFiles.length > 0
            ...
            <Panel
              header={
                <Space>
                  {renderHealthIcon(result.checks.index.healthy)}
                  <Text strong>索引状态</Text>
                  {renderHealthTag(result.checks.index.healthy)}
                </Space>
              }
              key="3"
            >
            ...
            <Panel
              header={
                <Space>
                  {renderHealthIcon(result.checks.mapping.healthy)}
                  <Text strong>映射关系</Text>
                  {renderHealthTag(result.checks.mapping.healthy)}
                </Space>
              }
              key="4"
            >
              <Descriptions bordered column={1}>
                ...
                <Descriptions.Item label="孤立文件数量">
                  <Text
                    strong
                    style={{ color: result.checks.mapping.orphanFiles.length > 0 ? '#ff4d4f' : '#52c41a' }}
                  >
                    {result.checks.mapping.orphanFiles.length}
                  </Text>
                ...
                <Descriptions.Item label="孤立记录数量">
                  <Text
                    strong
                    style={{ color: result.checks.mapping.orphanRecords.length > 0 ? '#ff4d4f' : '#52c41a' }}
                  >
                    {result.checks.mapping.orphanRecords.length}
                  </Text>
                ...
                <Descriptions.Item label="数据库不匹配数量">
                  <Text
                    strong
                    style={{ color: result.checks.mapping.databaseMismatch > 0 ? '#ff4d4f' : '#52c41a' }}
                  >
                    {result.checks.mapping.databaseMismatch}
                  </Text>
